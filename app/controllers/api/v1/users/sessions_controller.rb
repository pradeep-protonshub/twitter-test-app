  ...
  module V1
    module Users
      class SessionsController < Devise::SessionsController
        ...
        def facebook
          profile = Oauth::Facebook.new(params[:access_token]).build_user

          render_unprocessable_entity('invalid token') and return unless profile[:provider_id]
          render_unprocessable_entity('please grant email permission') and return unless profile[:email]

          @user = User.find_by(provider_id: profile[:provider_id]) rescue nil

          unless @user
            @user = User.new(profile)
            @user.skip_password_validation = true
            render_unprocessable_entity_response(@user) and return unless @user.save
          end

          sign_in :user, @user

          token, payload = Warden::JWTAuth::UserEncoder.new.call(@user, :json, 'JWT_AUD')
